...
const SearchContent = forwardRef(
  (
    ...
    // SSE data only updates ranks without triggering full search
    useEffect(() => {
      if (!sseData) return;

      const updateRanks = (ranksArray, RankClass, key) => {
        const itemID = sseData[key];
        if (!itemID || Number(sseData.senderID) !== message.id) return;

        const updatedRanks = ranksArray.filter((r) => r.id !== itemID);
        ...
        if (key === "movieID") {
          setMoviesRanks([...updatedRanks]);
        } else {
          setPeopleRanks([...updatedRanks]);
        }
      };

      if (sseData.movieID) {
        updateRanks(moviesRanks, Movie, "movieID");
      } else if (sseData.personID) {
        updateRanks(peopleRanks, Person, "personID");
      }
    }, [sseData, message.id, setMoviesRanks, setPeopleRanks]);
        ...
        <div className="resultContainer">
          {isLoading ? (
            ...
            <div className="results">
              {results.length > 0 && results.map((item, index) => <div key={index}>{item}</div>)}
            </div>
          )}
        </div>
      </div>
