document.addEventListener("DOMContentLoaded", () => {
    ...
    // Detect when the input is focused
    searchQuery.addEventListener('focus', function() {
        console.log('Input is active (focused)');
        searchContainer.style.border = '4px solid black'; // Change border when focused

        // Optionally, make the container fixed in one position
        searchContainer.style.position = 'fixed';  // You can adjust this based on your layout
        searchContainer.style.top = '10px';         // Position from the top
        searchContainer.style.left = '50%';         // Center horizontally
        searchContainer.style.transform = 'translateX(-50%)'; // Center the container
    });

    // Detect when the input loses focus
    searchQuery.addEventListener('blur', function() {
        console.log('Input is not active (blurred)');
        searchContainer.style.border = '2px solid black'; // Restore default border

        // Optionally, reset position when focus is lost
        searchContainer.style.position = 'relative'; // Return to normal flow
        searchContainer.style.top = 'initial';         // Reset position
        searchContainer.style.left = 'initial';        // Reset position
        searchContainer.style.transform = 'initial';   // Reset transform
    });
