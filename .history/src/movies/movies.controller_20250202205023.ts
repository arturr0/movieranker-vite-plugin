...
export class MoviesController {
        ...
        const movies = await Promise.all(results.map(async (movie) => {
          const ratings = await this.prisma.ratingMovie.findMany({
            ...
            select: {
              ...
              userId: true,
              userEmail: true,
              comment: true,  // Include the comment field
            },
          });
          ...
          return {
            ...
            id: movie.id,
            ratings, // Return ratings with comments
          };
        }));
  ...
  async rateItem(
    ...
    const { type, id, title, rating, post } = body;  // Destructure 'post' from the body
    ...
    if (type === 'movie') {
      ratingRecord = await this.prisma.ratingMovie.upsert({
        ...
        update: { rating, comment: post },  // Update with the 'post' value
        create: {
          userId,
          ...
          title,
          rating,
          comment: post,  // Create with the 'post' value
        },
      });
    } else if (type === 'person') {
