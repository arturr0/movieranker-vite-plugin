...
export class AuthService {
  ...
  async login(email: string, password: string): Promise<{ message: string; token: string }> {
    ...
    const payload = { id: user.id, email: user.email }; // Include necessary info
    const token = this.jwtService.sign(payload); // Create token
    return { message: 'Login successful', token }; // Return token
  }
