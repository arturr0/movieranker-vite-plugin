...
const RateContainer = ({ sseData, message, moviesRanks, peopleRanks, movieID, movieType, movieTitle, moviePoster, movieAvg, movieVotes, lastQuery }) => {
  ...
  useEffect(() => {
    if (sseData) {
      ...
      setMovieVotes(newVotes);  // Update votes count
      
      const newAvg = Math.round(newPosts.reduce((sum, r) => sum + r, 0) / item.ratings.length)
      
      setMovieAvg(newAvg);      // Update average rating
      // if (sseData.newPost) {
      //     setPosts((prevPosts) => {
