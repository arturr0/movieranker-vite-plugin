...
const SearchContent = () => {
  ...
  useEffect(() => {
    // Only fetch data when the query is updated and the user explicitly triggers the search (via Enter or magnifier)
    if (query) {
      setResults((prevResults) => ({
        ...prevResults,
        [type]: [], // Clear the results of the current type when a new search is initiated
      }));
    }
  }, [query, type]); // Only clear results when query or type changes
