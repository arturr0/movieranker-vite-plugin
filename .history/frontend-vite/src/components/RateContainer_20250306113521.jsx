...
const RateContainer = ({ message, movieId, movieType, movieTitle, moviePoster, movieAvg, movieVotes }) => {
  ...
  const rateItem = async (type, id, title, rating, post) => {
    // Add a check to ensure lastQuery is not empty
    if (!lastQuery || !lastQuery.type) {
      console.error("No lastQuery data available.");
      return;
    }
