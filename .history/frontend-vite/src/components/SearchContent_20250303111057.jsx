...
const SearchContent = () => {
  ...
  const handleRadioChange = (event) => {
    setSearchType(event.target.value); // Only update the search type
  };
  ...
  // Persist previous results when the type changes
  useEffect(() => {
    // Don't clear results when the radio button changes, just switch the type
    if (query.trim()) {
      searchMovies(); // Perform the search again when query or type changes
    }
