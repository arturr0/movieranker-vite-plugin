...
const SearchContent = () => {
  ...
  useEffect(() => {
    // Only clear results when query changes, not when the type changes
    if (query && !results[type].length) {
      setResults((prevResults) => ({
        ...prevResults,
        [type]: [], // Clear results of the current type when a new search is initiated
      }));
    }
  }, [query]); // Only listen to query change
   // Only clear results when query or type changes
