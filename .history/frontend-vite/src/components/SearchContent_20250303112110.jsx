...
const SearchContent = () => {
  ...
  const results = useRef({ title: [], actor: [] }); // UseRef to persist results without triggering re-renders
  ...
  const searchMovies = async () => {
    ...
    try {
      ...
      // Update the results for the current search type
      results.current[type] = data.movies || data.people || []; // Use the ref to update results without causing re-render
    } catch (error) {
    ...
    <div className="searchContent">
      ...
      <div className="resultContainer">
        ...
        <div className="results">
          ...
          {results.current[type].length > 0 ? (
            results.current[type].map((item, index) => (
