...
const searchMovies = useCallback(async () => {
  ...
  try {
    ...
    const processRanks = (items, type, targetArray) => {
      items.forEach((item) => {
        if (item.ratings) {
          item.ratings.forEach((rank) => {
            ...
            targetArray.push(rankedItem);
          });
        }
      });
    };

    processRanks(data.movies, 'movie', moviesRanks);
    processRanks(data.people, 'person', peopleRanks);
  } catch (error) {
